...
export default function StocksUsage() {
  ...
  const handleConfirm = async () => {
    if (!selectedRecipe) {
      validateText('', { required: true }, 'Receta');
      return;
    }

    // Validar cada insumo seleccionado
    for (const usage of supplyUsage) {
      const supply = selectedRecipe.supplies.find((s) => s._id === usage.supply);

      if (!supply) {
        toast.error(`Insumo no encontrado para el ID: ${usage.supply}`);
        return;
      }

      const isValid = validateGeneralNumber(
        usage.quantity,
        { required: true, isNegative: true },
        `Cantidad de ${supply.name}`
      );
                ...
                <>
                  {selectedRecipe.supplies.map((supply) => {
                    const currentQuantity =
                      supplyUsage.find((item) => item.supply === supply._id)?.quantity || 1;
                    return (
                      <Box className={classes.suppliesWrapper} key={supply._id}>
                        ...
                        <TextField
                          ...
                          error={Number(currentQuantity) <= 0}
                          helperText={
                          ...
                          onChange={(e) => {
                            ...
                            if (isNaN(numericValue) || numericValue <= 0) {
                              toast.error(`La cantidad para ${supply.name} debe ser mayor a 0`);
                              return;
                            }
                            handleChangeQuantity(value, supply._id);
                          }}
                        />
                      </Box>
                    );
